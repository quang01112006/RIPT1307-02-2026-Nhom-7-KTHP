...
import { CheckCircle2, Eye, EyeOff, Info, Lock, ShieldCheck } from 'lucide-react';
import React, { useState } from 'react';
import { history } from 'umi';

import { resetPassword as resetPasswordApi } from '@/services/base/api';
...
const ResetPasswordPage: React.FC = () => {
	...
	// Nhận email và otp từ trang ForgotPassword truyền qua
	const state = history.location.state as { email?: string; otp?: string } | undefined;
	...
	const onFinish = async (values: any) => {
		if (!state?.email || !state?.otp) {
			message.error('Thiếu thông tin xác thực! Vui lòng quay lại trang Quên mật khẩu.');
				...
				<div style={styles.card}>
					<div style={styles.cardBody}>
						<div style={styles.header}>
							...
							<div>
								<Title level={5} style={styles.brandName}>
									EduStack
								</Title>
						...
						<Text style={styles.subtitle}>
							Tạo mật khẩu mới an toàn để hoàn tất quá trình khôi phục tài khoản của bạn.
						</Text>
						...
						<Form layout='vertical' onFinish={onFinish} requiredMark={false}>
							...
							>
								<Input.Password
									...
									iconRender={(visible) =>
										visible ? <Eye size={18} color='#94a3b8' /> : <EyeOff size={18} color='#94a3b8' />
									}
								/>
							</Form.Item>
								...
								<div style={{ marginBottom: 18 }}>
									<div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
										<Text style={{ color: '#64748b', fontSize: 12 }}>
											Độ mạnh: <b style={{ color: strengthColor }}>{strengthText}</b>
										</Text>
							...
							<Button
								...
								htmlType='submit'
								loading={loading}
								style={{ ...styles.submit, background: '#10b981', borderColor: '#10b981' }}
							>
								CẬP NHẬT MẬT KHẨU <CheckCircle2 size={18} style={{ marginLeft: 8 }} />
							</Button>
						</Form>

						<div style={{ textAlign: 'center', marginTop: 18 }}>
							<a href='/user/login' style={{ color: '#2563eb', fontWeight: 600 }}>
