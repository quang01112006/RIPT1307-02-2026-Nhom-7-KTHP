...
export class PostsService {
  ...
  async findAll(query: PostQuery) {
    const { search, tag, page = 1, limit = 10, sort, author, includeHidden } = query as any;
    const filter: any = {};
  ...
  async update(
    ...
    const { title, content, tags, files, isHidden } = updatePostDto;

    if (typeof isHidden !== 'undefined' && !isAdmin) {
      throw new ForbiddenException('Bạn không có quyền thay đổi trạng thái ẩn bài viết');
    }

    const updateData: Record<string, any> = { title, content, tags, files };
